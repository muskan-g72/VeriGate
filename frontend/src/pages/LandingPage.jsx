...
const faqs = [
  ...
  ['Is it only for automated testing?', 'No. VeriGate supports automated, manual and exploratory verification—the goal is a complete record of what was checked and what the evidence proves.'],
...
export function LandingPage() {
  ...
  return <div className="landing-page">
    ...
    <main id="main-content">
      ...
      <section className="landing-section technology-section" id="technology"><div className="landing-container technology-layout"><div><p className="landing-eyebrow">BUILT FOR THE SIGNAL</p><h2>Your verification data,<br /><span>finally connected.</span></h2><p>A focused foundation for project structure, test intelligence, execution history, evidence and issues—designed to grow with your engineering organization.</p></div><div className="tech-stack"><div className="tech-core"><span><Zap /></span><small>VERIGATE CORE</small><strong>Traceability engine</strong></div>{['Projects','Test suites','Evidence','Issues'].map((item,index) => <div className={`tech-node tech-node--${index + 1}`} key={item}><i />{item}</div>)}</div></div><div className="landing-container trust-row">{['SECURITY-FIRST','API-READY','TRACEABLE','TEAM-CENTRIC','AUDIT-FRIENDLY'].map(x => <span key={x}>{x}</span>)}</div></section>
