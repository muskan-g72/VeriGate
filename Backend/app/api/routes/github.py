...
async def github_webhook(
    ...
    # 5. Find matching VeriGate project
    ...
    # 7. Check if verification enabled on project
    if not project.github_verification_enabled:
        response.status_code = status.HTTP_200_OK
        return {
            "status": "ignored",
            "reason": f"GitHub verification is disabled for project '{project.name}'.",
        }
    ...
    idempotency_key = (
        f"github_pr:{repo_full_name}:{pr_number}:{head_sha}:{action}"
    )
    ...
    # 11. Determine Test Suite to verify
    suites = list(
    ...
    # 12. Create VerificationRun with PR tracking metadata
    ...
    # 13. Queue asynchronous verification in background
    background_tasks.add_task(
        run_github_pr_verification, verification_run.id
    )

    logger.info(
        f"Queued PR verification {verification_run.id} for {repo_full_name} PR #{pr_number}@{short_sha}"
    )

    return {
        "status": "queued",
        "verification_run_id": str(verification_run.id),
        "run_id": str(verification_run.id),
        "project_id": str(project.id),
        "pr_number": pr_number,
        "commit_sha": head_sha,
        "test_count": len(selected_cases),
